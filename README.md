...
- **🛡️ Unsaved Session Protection**: Confirms and warns before switching away from an unsaved active profile.
- **📊 Real-Time Usage Dashboard**:
  - **5-hour session progress gauge** with live countdown timer.
  - **7-day weekly cap bar** & toolbar ring preview.
  - **Model usage breakdown** (Opus, Sonnet, Haiku, Claude Design).
  - **30-day activity heatmap** (7×24 hourly activity matrix).
- **🕒 Smart Predictive Peak Traffic Forecast**:
  ...
  - **Visual 24-Hour Local Timeline Bar** with color-coded traffic slots and live local time needle (`▼`).
  - Converts Claude's global peak schedule (5:00 AM – 1:00 PM PT) to your local timezone.
...
- **🔐 Encrypted Backup & Transfer**:
  ...
  - Seamless drag-and-drop JSON import.
- **🌓 Adaptive Theme**: Auto (System OS), Light, and Dark mode support.
- **🔒 Local-First & Zero Analytics**: Session tokens and cookies never leave your browser storage.
...
# 3. Build for all platforms (Chrome & Firefox)
npm run build
...
- `dist/` & `dist-chrome/` — Chrome / Chromium MV3 unpacked directory
...
├── scripts/                  # Cross-Browser Build & Packaging Scripts
...
MIT License. Crafted with precision for productivity on Claude.ai.
